...
namespace OnlineExamination.Repositories.Repository
{
    public class BatchParticipantRepository
    {
        ...
        public BatchParticipant GetParticipantByParticipantId(int participantId)
        {
            return db.BatchParticipants.FirstOrDefault(x => x.ParticipantId == participantId);
        }

        public bool Delete(int participantId)
        {
            BatchParticipant trainee = db.BatchParticipants.FirstOrDefault(x => x.ParticipantId == participantId);
            db.BatchParticipants.Remove(trainee);
            return db.SaveChanges() > 0;
        }
