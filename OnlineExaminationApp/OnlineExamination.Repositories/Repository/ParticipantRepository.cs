...
namespace OnlineExamination.Repositories.Repository
{
    public class ParticipantRepository
    {
        ...
        {
            return db.BatchParticipants.Where(x => x.BatchId == batchId).ToList();
        }

        public List<Participant> GetAllParticipantByCourseId(int courseId)
        {
            var baches = db.Batches.Where(x=>x.CourseId==courseId).ToList();

            return (from batch in baches from batchParti in db.BatchParticipants.Where(x => x.BatchId == batch.Id).ToList() select db.Participants.FirstOrDefault(x => x.Id == batchParti.ParticipantId)).ToList();
        }
    }
}