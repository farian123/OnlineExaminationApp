...
namespace OnlineExamination.Repositories.Repository
{
    public class CourseTrainerRepository
    {
        ...
        {
            CourseTrainer trainee = db.CourseTrainers.FirstOrDefault(x=>x.TraineeId==trainerId);
            db.CourseTrainers.Remove(trainee);
            return db.SaveChanges() > 0;
        }

        public CourseTrainer GetTrainerByTrainerId(int trainerId)
        {
            return db.CourseTrainers.FirstOrDefault(x => x.TraineeId == trainerId);
        }
    }
}
