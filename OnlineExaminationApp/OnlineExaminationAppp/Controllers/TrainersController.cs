...
namespace OnlineExaminationAppp.Controllers
{
    public class TrainersController : Controller
    {
        ...
        CourseTrainerManage courseTraineManage = new CourseTrainerManage();
        ...
        [HttpPost]
        public ActionResult CreateTrainerForCourse(TraineeViiewModel traineeViiewModel, HttpPostedFileBase image1,string ids)
        {
            if (ModelState.IsValid)
            {
                string fileName = Path.GetFileName(image1.FileName);
        ...
        [HttpPost]
        public ActionResult CreateTrainerForBatch(TraineeViiewModel traineeViiewModel, HttpPostedFileBase image1,string ids)
        {
            if (ModelState.IsValid)
            {
                string fileName = Path.GetFileName(image1.FileName);
        ...
        public ActionResult AddTrainerInBatch(BatchTrainerViewModel batchTrainerViewModel, string ids)
        {
            
            if (ModelState.IsValid)
            {
                BatchTrainer batchTrainer = batchTrainerManage.GetTrainerByTrainertId(batchTrainerViewModel.TraineeId);
                ...
                if (ids != null)
                {
                    ...
                    trainee = traineeManage.GetTraineeById(batchTrainerViewModel.TraineeId);
                    trainee.Lead = true;
        ...
        public ActionResult RemoveTrainerForFixedCourse(int id)
        {
            bool courseTrainer = courseTraineManage.Delete(id);
            if (courseTrainer)
            {
        ...
        public ActionResult RemoveTrainerForFixedBatch(int id)
        {
            bool batchTrainer = batchTrainerManage.Delete(id);
            if (batchTrainer)
            {
                var jsonResult = "Delete Trainer Successfull";
                return Json(jsonResult, JsonRequestBehavior.AllowGet);
